import { eq } from "drizzle-orm";
import type { Hono } from "hono";
import assert from "node:assert/strict";
import { db } from "../../src/db";
import {
  meters,
  motherMeterEvents,
  motherMeters,
  transactions,
  verification,
} from "../../src/db/schema";
import type { NewMeter, NewMotherMeter } from "../../src/db/schema";
import { uniqueRef } from "./helpers";

export async function loginAsLandlord(
  app: Pick<Hono, "request">,
  phoneNumber: string,
): Promise<string> {
  const sendResponse = await app.request("/api/mobile/landlord-access/send-otp", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ phoneNumber }),
  });
  assert.equal(sendResponse.status, 200);

  const otpVerification = await db.query.verification.findFirst({
    where: eq(verification.identifier, phoneNumber),
  });
  assert.ok(otpVerification);
  const [code] = otpVerification.value.split(":");
  assert.ok(code);

  const verifyResponse = await app.request("/api/mobile/landlord-access/verify-otp", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ code, phoneNumber }),
  });
  assert.equal(verifyResponse.status, 200);
  const verifyBody = (await verifyResponse.json()) as {
    data: { token: string };
  };

  return verifyBody.data.token;
}

export async function seedLandlordPropertyAnalyticsFixture(input: {
  customerId: string;
  fixtureMeterId: string;
  fixtureMotherMeterId: string;
  propertyId: string;
  tariffId: string;
}) {
  const [postpaidMotherMeter] = await db
    .insert(motherMeters)
    .values({
      landlordId: input.customerId,
      lowBalanceThreshold: "1000",
      motherMeterNumber: `MM-PROP-${uniqueRef("MM")}`,
      propertyId: input.propertyId,
      tariffId: input.tariffId,
      type: "postpaid",
    } satisfies NewMotherMeter)
    .returning({ id: motherMeters.id });
  const [postpaidMeter] = await db
    .insert(meters)
    .values({
      brand: "hexing",
      keyRevisionNumber: 1,
      meterNumber: `LANDLORD-PROP-POST-${uniqueRef("MTR")}`,
      meterType: "electricity",
      motherMeterId: postpaidMotherMeter.id,
      status: "active",
      supplyGroupCode: "600675",
      tariffId: input.tariffId,
      tariffIndex: 1,
    } satisfies NewMeter)
    .returning({ id: meters.id });

  await db.insert(motherMeterEvents).values([
    {
      amount: "300.00",
      createdAt: new Date("2026-03-11T06:00:00.000Z"),
      eventType: "initial_deposit",
      motherMeterId: input.fixtureMotherMeterId,
      performedBy: crypto.randomUUID(),
    },
    {
      amount: "120.00",
      createdAt: new Date("2026-03-12T09:00:00.000Z"),
      eventType: "bill_payment",
      motherMeterId: input.fixtureMotherMeterId,
      performedBy: crypto.randomUUID(),
    },
    {
      amount: "40.00",
      createdAt: new Date("2026-03-12T10:00:00.000Z"),
      eventType: "bill_payment",
      motherMeterId: postpaidMotherMeter.id,
      performedBy: crypto.randomUUID(),
    },
  ]);

  await db.insert(transactions).values([
    {
      amountPaid: "144.44",
      commissionAmount: "14.44",
      completedAt: new Date("2026-03-12T07:00:00.000Z"),
      meterId: input.fixtureMeterId,
      mpesaReceiptNumber: uniqueRef("MPESA"),
      netAmount: "130.00",
      paymentMethod: "paybill",
      phoneNumber: "254700111222",
      rateUsed: "24.0000",
      status: "completed",
      transactionId: uniqueRef("OHM"),
      unitsPurchased: "4.5000",
    },
    {
      amountPaid: "100.00",
      commissionAmount: "10.00",
      completedAt: new Date("2026-03-13T08:00:00.000Z"),
      meterId: postpaidMeter.id,
      mpesaReceiptNumber: uniqueRef("MPESA"),
      netAmount: "90.00",
      paymentMethod: "paybill",
      phoneNumber: "254711222333",
      rateUsed: "24.0000",
      status: "completed",
      transactionId: uniqueRef("OHM"),
      unitsPurchased: "3.0000",
    },
  ]);

  return { postpaidMotherMeterId: postpaidMotherMeter.id };
}
