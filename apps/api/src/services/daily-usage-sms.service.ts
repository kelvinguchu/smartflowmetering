...
function getDateBoundsInTimezone(
  dateStr: string,
  timezone: string,
  ...
  if (localDay > day || (localDay === 1 && day > 27)) {
    offsetHours = localHour;
  } else if (localDay < day || (localDay > 27 && day === 1)) {
    offsetHours = localHour - 24;
  }

  const dayStart = new Date(rough.getTime() - offsetHours * 3_600_000);
