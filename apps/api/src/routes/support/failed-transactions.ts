...
import { loadFailedTransactionReviewHistory } from "../../services/support/failed-transaction-review-history.service";
import {
  getFailedTransactionWorkflowGuidance,
  validateFailedTransactionStatusUpdate,
} from "../../services/support/failed-transaction-policy.service";
...
failedTransactionRoutes.get(
  ...
  async (c) => {
    ...
    const reviewHistoryByFailedTransactionId =
      await loadFailedTransactionReviewHistory(rows.map((row) => row.id));
    ...
    return c.json({
      ...
      data: rows.map((row) => {
        ...
        const reviewHistory =
          reviewHistoryByFailedTransactionId.get(row.id) ?? [];
...
failedTransactionRoutes.patch(
  ...
  async (c) => {
    ...
    const reviewHistory = await loadFailedTransactionReviewHistory([id]);
    const reviews = reviewHistory.get(id) ?? [];
