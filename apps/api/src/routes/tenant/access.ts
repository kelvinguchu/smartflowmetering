...
} from "../../services/customer/customer-device-tokens.service";
import {
  acknowledgeTenantTokenDelivery,
  bootstrapTenantAccess,
  listTenantNotifications,
  markTenantNotificationRead,
} from "../../services/tenant/tenant-access.service";
import {
  toPublicCustomerDeviceToken,
  toPublicTenantAccess,
} from "../../services/customer/mobile-public-response.service";
import {
  getTenantDashboardSummary,
  listTenantPurchases,
} from "../../services/tenant/tenant-dashboard.service";
import { getTenantExceptionalState } from "../../services/tenant/tenant-exceptional-state.service";
import { getTenantHistorySummary } from "../../services/tenant/tenant-history-summary.service";
import { listTenantPurchaseRollups } from "../../services/tenant/tenant-purchase-rollups.service";
import { listTenantRecoveryStates } from "../../services/tenant/tenant-recovery-state.service";
import {
  getTenantTokenDeliveryDetail,
  listTenantTokenDeliveries,
} from "../../services/tenant/tenant-token-delivery.service";
import {
  ...
  async (c) => {
    ...
    return c.json({
      data: {
        ...
        tenantAccess: toPublicTenantAccess(result.tenantAccess.meter),
      },
...
tenantAccessRoutes.get("/me", (c) =>
  c.json({
    data: toPublicTenantAccess(c.get("tenantAccess")),
  }),
);
...
tenantAccessRoutes.post(
  ...
  async (c) => {
    ...
    return c.json({
      ...
      message: "Tenant device token saved",
    });
  },
);
